...
public class MappedFileQueue {
     ...
     * 如果当前时间大于文件的最大存活时间或需要强制删除文件（当磁盘使用超过设定的阈值）时则执行MappedFile#destory方法，清除MappedFile占有的相关资源，
     * 如果执行成功，将该文件加入到待删除文件列表中，然后统一执行File#delete方法将文件从物理磁盘中删除。
     * @param expiredTime
    ...
    /**
     * 根据消息偏移量offset查找MappedFile。根据offset查找MappedFile直接使用offset%-mapped FileSize是否可行？答案是否定的，由于使用了内存映射，
